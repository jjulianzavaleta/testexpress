...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'woo-cart-weight'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'woo-cart-weight'), 'product_status' => \__('Subscription Status', 'woo-cart-weight'), 'product_license' => \__('Subscription Data', 'woo-cart-weight')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'woo-cart-weight');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'woo-cart-weight');
        }
        return $status;
    }
