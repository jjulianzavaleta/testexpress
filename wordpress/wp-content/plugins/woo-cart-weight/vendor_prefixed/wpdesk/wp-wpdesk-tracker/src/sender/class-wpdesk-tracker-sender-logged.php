    ...
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        public function send_payload(array $payload)
        {
            if (\class_exists('WCWeightVendor\\WPDesk_Logger_Factory')) {
                \WCWeightVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \WCWeightVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \WCWeightVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \WCWeightVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\WCWeightVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \WCWeightVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
