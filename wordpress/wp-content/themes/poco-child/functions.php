...
function recalculate_bags_on_cart() {
  if(!is_admin() && is_user_logged_in()) {
    ...
    $roles = ( array ) $user->roles;
		...
		$url_api_product_stock = ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "http://10.152.1.15:8080/api/GET/product_stock.php?" : "../wp-content/themes/poco-child/product_stock.php?";
				...
				foreach ( WC()->cart->get_cart() as $cart_item ) {
					...
					if( $item_sku != '101578'){
						...
						if(!empty($shipping_delivery_type) && !empty($shipping_state)) {
							...
							$message = 'Por favor seleccione el lugar de entrega en la parte superior 1';
						}
					}
				}
				if(!$valid){
...
function woocommerce_add_to_cart_stock_validation( $passed, $product_id, $quantity, $variation_id = null, $variations = null ) {
	$current_user = wp_get_current_user();
	$url_api_product_stock = ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "../wp-content/themes/poco-child/product_stock.php?" : "../wp-content/themes/poco-child/product_stock.php?";
	...
	if($current_user->ID != 0){
		if(!empty($shipping_delivery_type) && !empty($shipping_state)) {
			...
			wc_add_notice( __( 'Por favor seleccione el lugar de entrega en la parte superior', 'woocommerce' ), 'notice' );
			$passed = false;
		}
	} else {
		// $message = '<a href="' . site_url('/my-account/') . '">Por favor haga click aquí para iniciar sesión o registrarse</a>';
...
/**
  * Añadir mensaje de inicio de sesión en productos individuales
  */

function single_product_alert() {
	$current_user = wp_get_current_user();

	if($current_user->ID == 0){
		wc_clear_notices();
		echo "<div class='woocommerce-info' style='background-color: #ff7200;'><a href='" . site_url('/my-account/') . "' style='color: var(--primary)'>Para continuar con su pedido, por favor hacer click aquí para iniciar sesión o registrarse</a></div>";
	}
}

add_action( 'woocommerce_before_single_product', 'single_product_alert', 20 );
...
if(!function_exists('custom_question_conditional_javascript')) {
	function custom_question_conditional_javascript() {
		...
		// 	console.log("a");
		// 	var elements = document.getElementsByClassName("product-caption");
		// 	for (let i = 0; i < elements.length; i++) {
		// 		// elements[i].style.pointerEvents = "none";
		// 		// elements[i].style.opacity = "0.5";
		// 		window.jQuery(elements[i]).attr("disabled","disabled");
		// 	}
		// }
	...
	<div class="delivery_details">
		...
		<script>
			var ajax_url = <?php if ($_SERVER['SERVER_NAME'] == "10.152.1.15"){
				echo "'http://10.152.1.15:8080/wp-admin/admin-ajax.php'";
...
function custom_processing($order_id) {
  ...
  if ($_SERVER['SERVER_NAME'] == "10.152.1.15"){
    $urlAddToTempTablesPOS = "http://10.152.1.15:8080/api/POST/addOrderToTempTablesPOS.php";
  } else {
...
function after_order_update($order_id){
  ...
  if ($_SERVER['SERVER_NAME'] == "10.152.1.15"){
    $urlAddToTempTablesPOS = "http://10.152.1.15:8080/api/PUT/updateOrderToTempTablesPOS.php";
  } else {
...
<script>
  ...
  gtag('config', 'G-139PWT5D6R');
	...
	const url_api_person_company = <?php echo ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "'http://10.152.1.15:8080/wp-content/themes/poco-child/person_company.php'" : "'https://xpress.chimuagropecuaria.com.pe/wp-content/themes/poco-child/person_company.php'"; ?>;
	const url_api_product_stock = <?php echo ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "'http://10.152.1.15:8080/wp-content/themes/poco-child/product_stock.php'" : "'https://xpress.chimuagropecuaria.com.pe/wp-content/themes/poco-child/product_stock.php'"; ?>;
	const url_update_usermeta = <?php echo ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "'http://10.152.1.15:8080/wp-content/themes/poco-child/update_usermeta.php'" : "'https://xpress.chimuagropecuaria.com.pe/wp-content/themes/poco-child/update_usermeta.php'"; ?>;

	const id_login_popup = <?php echo ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "13549" : "14859"; ?>;
	const id_shipping_popup = <?php echo ($_SERVER['SERVER_NAME'] == "10.152.1.15") ? "13489" : "14899"; ?>;
</script>
...
fbq('init', '385976059457245');
fbq('track', 'PageView');
</script>
<noscript><img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=385976059457245&ev=PageView&noscript=1"
/></noscript>
