...
class Poco_Megamenu {
	...
	public function __construct() {
		...
		if ( $this->check_megamenu() ) {
			...
			add_filter( 'poco_nav_menu_args', [ $this, 'set_menu_args' ], 99999 );
		}

	}

	public function set_menu_args( $args ) {
		$args['walker'] = new Poco_Megamenu_Walker();
	...
	private function check_megamenu() {
		...
		foreach ( $all as $menu_item ) {
			$elementor_id = poco_megamenu_get_post_related_menu( $menu_item->ID );
			if ( $elementor_id ) {
	...
	public function enqueue_scripts() {
		global $poco_version;
		wp_enqueue_script( 'poco-megamenu-frontend', get_template_directory_uri() . '/inc/megamenu/assets/js/frontend.js', array( 'jquery' ), $poco_version, true );
