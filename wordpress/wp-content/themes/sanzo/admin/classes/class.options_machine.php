...
class Sanzo_Options_Machine {
	...
	public static function optionsframework_machine($options) {
		global $sanzo_smof_output, $sanzo_smof_details, $sanzo_theme_options;
		if (empty($options))
			return;
		if (empty($sanzo_theme_options))
		...
		do_action('sanzo_optionsframework_machine_before', array(
				'options'	=> $options,
				'smof_data'	=> $sanzo_theme_options,
			));
		if ($sanzo_smof_output != "") {
			$output .= $sanzo_smof_output;
			$sanzo_smof_output = "";
		}
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($sanzo_theme_options) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($sanzo_theme_options[$value['id']])) {
					if ($value['type'] != "info") {
						$sanzo_theme_options[$value['id']] = $value['std'];
						if ($value['type'] == "checkbox" && $value['std'] == 0) {
							$sanzo_theme_options[$value['id']] = 0;
						} else {
							$update_data = true;
						}
					}
				}
				if (array_key_exists('id', $value) && !isset($sanzo_smof_details[$value['id']]) && $value['type'] != "info") {
					$sanzo_smof_details[$value['id']] = $sanzo_theme_options[$value['id']];
				}
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($sanzo_theme_options[$value['fold']]) && $sanzo_theme_options[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			//if (!isset($sanzo_theme_options[$value['id']]) && $value['type'] != "heading")
			//	continue;
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($sanzo_theme_options[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($sanzo_theme_options[$value['id']], $theValue, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($sanzo_theme_options[$value['id']]);			
				...
				case 'css_field':
					...
					if(isset($value['options'])){
						...
						$ta_value = $sanzo_theme_options[$value['id']];			
				...
				//radiobox option
				case "radio":
					$checked = (isset($sanzo_theme_options[$value['id']])) ? checked($sanzo_theme_options[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($sanzo_theme_options[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($sanzo_theme_options[$value['id']])) {
						$sanzo_theme_options[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($sanzo_theme_options[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($sanzo_theme_options[$value['id']]))? $multi_stored = $sanzo_theme_options[$value['id']] : $multi_stored="";
				...
				case "color":
					...
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $sanzo_theme_options[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					$typography_stored = isset($sanzo_theme_options[$value['id']]) ? $sanzo_theme_options[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $sanzo_theme_options[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($sanzo_theme_options[$value['id']])) ? $sanzo_theme_options[$value['id']] : '';
				...
				case 'slider':
					...
					$slides = $sanzo_theme_options[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Sanzo_Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">'.esc_html__('Add New Slide', 'sanzo').'</a></div>';
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($sanzo_theme_options[$value['id']]) && !empty($sanzo_theme_options[$value['id']]) ? $sanzo_theme_options[$value['id']] : '';
				...
				case 'backup':
					...
					$backup = sanzo_of_get_options(BACKUPS);
					$init = sanzo_of_get_options('smof_init');


					if(!isset($backup['backup_log'])) {
						$log = esc_html__('No backups yet', 'sanzo');
					} else {
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ', 'sanzo').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="'.esc_html__('Backup Options', 'sanzo').'">'.esc_html__('Backup Options', 'sanzo').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="'.esc_html__('Restore Options', 'sanzo').'">'.esc_html__('Restore Options', 'sanzo').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.json_encode($sanzo_theme_options).'</textarea>'."\n";
					$output .= '<a href="#" id="of_import_button" class="button" title="'.esc_html__('Import Options', 'sanzo').'">'.esc_html__('Import Options', 'sanzo').'</a>';
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($sanzo_theme_options[$value['id']]))? $sanzo_theme_options[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
					...
					$hide = " hide";
					if ($sanzo_theme_options[$value['id']] != "none" && $sanzo_theme_options[$value['id']] != "")
						$hide = "";
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($sanzo_theme_options[$value['id']]);
				...
				case 'switch':
					...
					//Get selected
					if ($sanzo_theme_options[$value['id']] == 1){
						$cb_enabled = ' selected';
						$cb_disabled = '';
					}else{
						$cb_enabled = '';
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($sanzo_theme_options[$value['id']], 1, false) .' />';
				...
				case "button":
					...
					$output .= '<a href="javascript: void(0)" class="ts-button">'.$button_text.'</a>';
				break;
				...
				case "media":
					...
					$u_val = '';
					if($sanzo_theme_options[$value['id']]){
						$u_val = stripslashes($sanzo_theme_options[$value['id']]);
					}

					$output .= Sanzo_Options_Machine::optionsframework_media_uploader_function($value['id'],$u_val, $value['mod']);
	    ...
	    do_action('sanzo_optionsframework_machine_after', array(
					'options'		=> $options,
					'smof_data'		=> $sanzo_theme_options,
					'defaults'		=> $defaults,
					'counter'		=> $counter,
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		$uploader = '';
		$upload = "";
		if (isset($sanzo_theme_options[$id]))
	    	$upload = $sanzo_theme_options[$id];
		$hide = '';
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.esc_html__('Upload', 'sanzo').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.esc_html__('Remove', 'sanzo').'</span>';
		}
		else 
		{
			$output .= '<p class="upload-notice"><i>'.esc_html__('Upgrade your version of WordPress for full media support.', 'sanzo').'</i></p>';
		}
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		$slider .= '<a class="slide_edit_button" href="#">'.esc_html__('Edit', 'sanzo').'</a></div>';
		
		$slider .= '<div class="slide_body">';
		
		$slider .= '<label>'.esc_html__('Title', 'sanzo').'</label>';
		...
		$slider .= '<label>'.esc_html__('Image URL', 'sanzo').'</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">'.esc_html__('Upload', 'sanzo').'</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">'.esc_html__('Remove', 'sanzo').'</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';	
		$slider .= '<label>'.esc_html__('Link URL (optional)', 'sanzo').'</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
		
		$slider .= '<label>'.esc_html__('Description (optional)', 'sanzo').'</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<a class="slide_delete_button" href="#">'.esc_html__('Delete', 'sanzo').'</a>';
